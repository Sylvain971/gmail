...
def get_the_email_of_a_townhall_from_its_webpage
	doc = Nokogiri::HTML(open("http://annuaire-des-mairies.com/971/anse-bertrand.html"))
	...
	email_anse_bertrand = doc.css('p[class = "Style22"]')[11].text
end
...
def get_all_the_urls_of_guadeloupe_townhalls
	doc = Nokogiri::HTML(open("http://annuaire-des-mairies.com/guadeloupe.html"))
	...
	doc.css("a[class = 'lientxt']").each do |url|
		...
		$list_of_url_971 << url["href"]
	end
end	

get_all_the_urls_of_guadeloupe_townhalls


#Je crée une méthode pour récupérer toutes les adresses emails de toutes les mairies
def get_all_the_emails_of_guadeloupe_townhalls
	$data_guadeloupe = Hash.new
	$list_of_url_971.each do |url|
		url = "http://annuaire-des-mairies.com/" + url
		doc = Nokogiri::HTML(open(url))
		town = doc.css('a[class = "lientxt4"]')[0].text
		email = doc.css('p[class = "Style22"]')[11].text
		$data_guadeloupe[town] = email
	end
end

get_all_the_emails_of_guadeloupe_townhalls