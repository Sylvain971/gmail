...
def get_all_the_urls_of_reunion_island_townhalls
	doc = Nokogiri::HTML(open("http://annuaire-des-mairies.com/reunion.html"))
	...
	doc.css("a[class = 'lientxt']").each do |url|
		...
		$list_of_url_974 << url["href"]
	end
end	

get_all_the_urls_of_reunion_island_townhalls

#Je crée une méthode pour récupérer toutes les adresses emails de toutes les mairies
def get_all_the_emails_of_reunion_island_townhalls
	$data_reunion_island = Hash.new
	$list_of_url_974.each do |url|
		url = "http://annuaire-des-mairies.com/" + url
		doc = Nokogiri::HTML(open(url))
		town = doc.css('a[class = "lientxt4"]')[0].text
		email = doc.css('p[class = "Style22"]')[11].text
		$data_reunion_island[town] = email
	end
end

get_all_the_emails_of_reunion_island_townhalls